...
def train_with_tracking(model: AdvancedNeuralNetwork,
    ...
    training_results = model.train_with_custom_constraints(X_train,
        y_train, X_val, y_val,
    ...
    # Save loss history
    a='adaptive_loss_history'
    if 'adaptive_loss_history' in training_results and training_results[a]:
        try:
    ...
    if model.performance_tracker:
        per=model.performance_tracker.get_summary()
    else:
        per={}
    return {'training': training_results, 'test': test_results,
        'performance_summary': per, 
...
def display_results(results: Dict[str, Any]) -> None:
    ...
    try:
        ...
        print(f"{performance_summary.get('avg_epoch_time', 'N/A'):.2f}s")
        print("Peak memory:")
        print(f"{performance_summary.get('peak_memory_mb', 'N/A'):.1f} MB")
        print("\n🧪 Test Results:")
...
def main():
    ...
    try:
        ...
        print("  Features: {X_train.shape[1]} -> {y_train.shape[1]}")
    except Exception as e:
    ...
    try:
        model = AdvancedNeuralNetwork((X_train.shape[1],),
                                      y_train.shape[1], model_config)
    ...
    try:
        results = train_with_tracking(model, X_train, X_val, X_test,
