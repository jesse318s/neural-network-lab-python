...
def datestamp(x):
    try:
        return datetime.fromtimestamp(x).isoformat() if x else 'Unknown'
    except:
        return str("===="+x+"==\n")
...
def _save_training_log(self): """Save detailed training log to text file."""
        try:
            ...
            with open(file_path, 'w', encoding='utf-8') as logfile:
                ...
                logfile.write(f"Training started: {datestamp(self.training_start_time)}\n")
                logfile.write(f"Training ended: {datestamp(self.training_end_time)}\n")
